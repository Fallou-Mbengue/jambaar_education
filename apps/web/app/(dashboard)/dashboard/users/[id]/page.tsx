    ...
    <div className="p-6 space-y-6">
      ...
      <div className="glass rounded-xl p-6 grid grid-cols-2 md:grid-cols-4 gap-6">
        <div className="text-center">
          ...
          <p className="text-2xl font-bold text-white">{gamification?.totalXp ?? 0}</p>
        ...
        <div className="text-center">
          ...
          <p className="text-2xl font-bold text-white">{gamification?.streak ?? 0}</p>
        ...
        <div className="text-center">
          ...
          <p className="text-2xl font-bold text-white">{progress?.completedContent ?? 0}</p>
          <p className="text-dark-text text-xs">Contenus terminés</p>
        </div>
        <div className="text-center">
          <p className="text-2xl font-bold text-white capitalize">{gamification?.currentLevel?.toLowerCase() ?? '-'}</p>
      ...
      {softSkillScores && Object.keys(softSkillScores).length > 0 && (
        <div className="glass rounded-xl p-6">
          <h2 className="text-white font-semibold mb-4">Scores par compétence</h2>
          ...
          <div className="space-y-3">
            {progress.recent.map((item: any) => (
              <div key={item.id} className="flex items-center gap-3 text-sm">
                ...
                <span className="text-white flex-1">{item.content?.title ?? 'Contenu'}</span>
